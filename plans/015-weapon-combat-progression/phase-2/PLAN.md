...
### Final essential gate —13 September2026

Phase2's candidate ownership/UI boundary is verified. Browser gate PASSED on0.113.5/a2608b3: exact Luna request from School opens collection; weapon artwork loads through the real symlinked installation; art/HUD clicks and numbered actions select owned instances; duplicate ID refused; two distinct Common copies work; deposit double-click pays once;390px layout inspected. School retains non-slot training. A paid lesson was unaffordable in the honest returning-player fixture (2XP,45gold; minimum20XP,8gold), so that browser purchase remains in phase3/7; coded spending checks pass. Report: `dojo/results/0065-015-collection-2026-09-13/summary.md`. The full migration/compensation/active-fight matrix remains phase7, as planned.

Browser-driven fixes: `eede110` binds every action button, `c1de8e7` fixes art URLs when the package is symlinked, `a2608b3` prevents an empty candidate deck from receiving old missing-starter compensation. Their final targeted collection/static-art/opener checks pass:26 tests in2.38s. The final full plugin run finished with1458 passed,9 failed,3 skipped,1 expected failure in657.71s under exceptional host load. Failure identities shifted across the06:00UTC world-day boundary; the three newly observed failures were reproduced unchanged against archived0.112.0 in2.00s (`/tmp/ascent-phase2-legacy-followup.txt`): slowing-arrow chase math, foe-sheet specimen speed and first-sighting kill. All legacy failures remain release-gate work. Do not describe this as a fully passing suite.

Plugin code rollback before candidate writes: `git revert a2608b3 c1de8e7 eede110 46c432d f73d853 0ebba87`. Parent implementation began at9cb5646; latest vendor-fix commit is recorded by git history. After writes, retain readers/receipts and stop enrollment instead of restoring snapshots. All changes remain QA-only. Proceed to phase3's committed shared-resolver contract.
