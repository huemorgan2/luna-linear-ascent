...
Enemies resolve sequentially. A kill pays XP once immediately, copies rolled rewards into pending haul and reveals the next member at cover distance; it does not heal, reset cooldowns or charge the next member. Final victory transfers the entire haul exactly once. Materials live in their dedicated store. Weapon instances retain a carrying/storage location and share carried pack capacity outside the three deck cells. A full pack secures new weapons in a persistent victory claim; resolve that claim before another hunt, never delete it or turn it into unlimited remote inventory. Encounter objectives and external kill notifications receive completion only after the full group; preserve immediate per-kill XP in the engine ledger. The result records kill count, paid energy, exhaustion and secured/lost haul for clients and simulator accounting.
