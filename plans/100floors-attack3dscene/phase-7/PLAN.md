# Phase 7 — floors 61–70: full asset generation + gate to 1–70

## Goal

The arena (and the kill finisher) runs with full 3D assets on floors
61–70: 50 foe ids (40 encounters + wardens 061–070) each get a
monster GLB, a 320×112 kill background, and a 320×300 arena backdrop.
Gate widened to floors 1–70; suites green; dojo PASS; deployed.
...
1. **Author prompts** (the human-scale work, from the floor YAMLs'
   ...
   - `research/3d-fight/gen_bg_floors.py`: 10 new `FLOOR` settings
     (floors 61–70) + 50 `SCENES` entries (wind/fire params per scene,
...
5. **Arena backdrops**: `gen_bg_arena.py` floor range 61–70; canary,
   `stills` (50) + `sheets` into `backgrounds300/`.
6. Gate: `READY_FLOORS = frozenset(range(1, 71))` in `engine/arena.py` (plugin submodule); boundary tests:
   enabled on 61 and 70, disabled on 71. Pre-rsync diff, then `worldd/tools/vendor_game.sh`.
...
9. Dojo: arena with backdrop + 3D monster on floors 61 and 70;
   classic 2D card on floor 71. Results under `dojo/results/`.
10. Deploy (`worldd/tools/deploy.sh`), verify a floor-61 fight live.
    Append Execution status here; commit.

## Verification

- Census: 0 ids missing for floors 61–70 across `monsters/`,
...
- Gate boundary unit test: enabled on 61 and 70, disabled on 71.
- Dojo results folder with PASS rows + screenshots; no 404s on
  `monsters/<id>.glb` or the two background dirs during the fights.
- Live fight on floor 61 post-deploy shows monster + backdrop in 3D.

## Rollback

`git revert` the gate commits (plugin + workspace) and redeploy — floors
61–70 fall back to the classic card. Shipped assets stay (inert
