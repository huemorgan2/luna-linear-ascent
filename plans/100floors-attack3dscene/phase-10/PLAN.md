# Phase 10 — floors 91–100: full asset generation + gate to 1–100

## Goal

The arena (and the kill finisher) runs with full 3D assets on floors
91–100: 50 foe ids (40 encounters + wardens 091–100) each get a
monster GLB, a 320×112 kill background, and a 320×300 arena backdrop.
Gate deleted — the arena is unconditionally on; suites green; dojo PASS; deployed.
...
1. **Author prompts** (the human-scale work, from the floor YAMLs'
   ...
   - `research/3d-fight/gen_bg_floors.py`: 10 new `FLOOR` settings
     (floors 91–100) + 50 `SCENES` entries (wind/fire params per scene,
...
5. **Arena backdrops**: `gen_bg_arena.py` floor range 91–100; canary,
   `stills` (50) + `sheets` into `backgrounds300/`.
6. Gate: delete `READY_FLOORS` and the gate — `enabled()` returns True whenever an encounter floor resolves (the promotion contract's final step) in `engine/arena.py` (plugin submodule); boundary tests:
   no boundary left — enabled on every floor with an encounter. Pre-rsync diff, then `worldd/tools/vendor_game.sh`.
...
9. Dojo: arena with backdrop + 3D monster on floors 91 and 100;
   every floor now 3D — spot-check one floor per earlier phase still healthy. Results under `dojo/results/`.
10. Deploy (`worldd/tools/deploy.sh`), verify a floor-91 fight live.
    Append Execution status here; commit.
11. Close out the main plan: mark the rollout complete, delete the phase-gate comment from `arena.py`, and sweep for stale references to the Labs arena in comments/docs.

## Verification

- Census: 0 ids missing for floors 91–100 across `monsters/`,
...
- Gate boundary unit test: no boundary left — enabled on every floor with an encounter.
- Dojo results folder with PASS rows + screenshots; no 404s on
  `monsters/<id>.glb` or the two background dirs during the fights.
- Live fight on floor 91 post-deploy shows monster + backdrop in 3D.

## Rollback

`git revert` the gate commits (plugin + workspace) and redeploy — floors
91–100 fall back to the classic card (the Labs gate does NOT come back; reverting to the phase-9 gate keeps floors 1–90 live). Shipped assets stay (inert
