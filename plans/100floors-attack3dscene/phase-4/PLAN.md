# Phase 4 — floors 31–40: full asset generation + gate to 1–40

## Goal

The arena (and the kill finisher) runs with full 3D assets on floors
31–40: 51 foe ids (41 encounters + wardens 031–040) each get a
monster GLB, a 320×112 kill background, and a 320×300 arena backdrop.
Gate widened to floors 1–40; suites green; dojo PASS; deployed.
...
1. **Author prompts** (the human-scale work, from the floor YAMLs'
   ...
   - `research/3d-fight/gen_bg_floors.py`: 10 new `FLOOR` settings
     (floors 31–40) + 51 `SCENES` entries (wind/fire params per scene,
     wardens get the `GATE` prefix).
   - `research/3d-fight/3d models/gen_floors.py`: 51 creature prompts
...
4. **Kill backgrounds**: canary one still, then `gen_bg_floors.py stills`
   (51) + `gifs`, then `make_bg_sheets.py` bakes into
   `worldd/static/site/fight3d/backgrounds/`.
5. **Arena backdrops**: `gen_bg_arena.py` floor range 31–40; canary,
   `stills` (51) + `sheets` into `backgrounds300/`.
6. Gate: `READY_FLOORS = frozenset(range(1, 41))` in `engine/arena.py` (plugin submodule); boundary tests:
   enabled on 31 and 40, disabled on 41. Pre-rsync diff, then `worldd/tools/vendor_game.sh`.
...
9. Dojo: arena with backdrop + 3D monster on floors 31 and 40;
   classic 2D card on floor 41. Results under `dojo/results/`.
10. Deploy (`worldd/tools/deploy.sh`), verify a floor-31 fight live.
    Append Execution status here; commit.

## Verification

- Census: 0 ids missing for floors 31–40 across `monsters/`,
...
- Gate boundary unit test: enabled on 31 and 40, disabled on 41.
- Dojo results folder with PASS rows + screenshots; no 404s on
  `monsters/<id>.glb` or the two background dirs during the fights.
- Live fight on floor 31 post-deploy shows monster + backdrop in 3D.

## Rollback

`git revert` the gate commits (plugin + workspace) and redeploy — floors
31–40 fall back to the classic card. Shipped assets stay (inert
