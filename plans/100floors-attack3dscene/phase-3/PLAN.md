# Phase 3 — floors 21–30: full asset generation + gate to 1–30

## Goal

The arena (and the kill finisher) runs with full 3D assets on floors
21–30: 50 foe ids (40 encounters + wardens 021–030) each get a
monster GLB, a 320×112 kill background, and a 320×300 arena backdrop.
Gate widened to floors 1–30; suites green; dojo PASS; deployed.
...
1. **Author prompts** (the human-scale work, from the floor YAMLs'
   ...
   - `research/3d-fight/gen_bg_floors.py`: 10 new `FLOOR` settings
     (floors 21–30) + 50 `SCENES` entries (wind/fire params per scene,
...
5. **Arena backdrops**: `gen_bg_arena.py` floor range 21–30; canary,
   `stills` (50) + `sheets` into `backgrounds300/`.
6. Gate: `READY_FLOORS = frozenset(range(1, 31))` in `engine/arena.py` (plugin submodule); boundary tests:
   enabled on 21 and 30, disabled on 31. Pre-rsync diff, then `worldd/tools/vendor_game.sh`.
...
9. Dojo: arena with backdrop + 3D monster on floors 21 and 30;
   classic 2D card on floor 31. Results under `dojo/results/`.
10. Deploy (`worldd/tools/deploy.sh`), verify a floor-21 fight live.
    Append Execution status here; commit.

## Verification

- Census: 0 ids missing for floors 21–30 across `monsters/`,
...
- Gate boundary unit test: enabled on 21 and 30, disabled on 31.
- Dojo results folder with PASS rows + screenshots; no 404s on
  `monsters/<id>.glb` or the two background dirs during the fights.
- Live fight on floor 21 post-deploy shows monster + backdrop in 3D.

## Rollback

`git revert` the gate commits (plugin + workspace) and redeploy — floors
21–30 fall back to the classic card. Shipped assets stay (inert
