...
class GameConfig:
    ...
    def from_dict(cls,values):
        ...
        ints=dict(players=(1,100000),days=(1,3650),seed=(0,2147483647),workers=(0,4096),max_floor=(1,100),
            sessions_per_day=(1,12),readiness_trials=(2,32),probe_every_days=(1,30),max_combat_actions=(10,500),trace_players=(0,100),world_frontier=(0,100))
...
def assess(document,seconds,floor,cfg):
    ...
    for trial in range(cfg.readiness_trials):
        ...
        for action in range(cfg.max_combat_actions):
            s.act(fight_choice(s,'tactician',probe=True))
...
class Agent:
    ...
    def probe(self):
        ...
        self.probe_key=key
        f=self.ready+1
        assessment=assess(p,self.s.seconds,f,self.cfg);self.last_probe=assessment
        if assessment['win_rate']>=self.cfg.readiness_threshold:
            self.ready=f
    ...
    def run(self):
        for day in range(self.cfg.days):
            ...
            for visit in range(self.cfg.sessions_per_day):
                ...
                if visit==self.cfg.sessions_per_day-1 and not self.s.doc.get('encounter'):
                    self.s.look()
        ...
        return dict(id=self.id,policy=self.policy,ready_floor=self.ready,milestones=self.milestones,floors=self.floors,
            ...
            last_probe=self.last_probe,recent_actions=self.recent,final=p,state_sha256=self.s.state_hash(),
