...
`python simulation/verification/009/targeted_niches.py --seeds 16 --out simulation/verification/009/targeted-results.json` uses the same core with explicit honed initial preparation. Source0.115.1 is recorded in the immutable output. Ramguard→Hawkeye has64/64 wins in both modes, with57 paired improvements in health/actions. Frostbind without speed shoes wins52/64 versus33/64 plain,44 improvements. Runestring Arcane wins80/112 versus68/112 Ordinary,64 improvements, but loses32 previously won AirPower fights. The arcane choice is situational; it must not become an automatic upgrade over ordinary arrows. These are conditional combat comparisons, not evidence for elapsed days, affordable earning or an optimum.

`python simulation/verification/009/passive_niches.py` runs384 actual first actions on matched real creatures,64pairs per comparison. Breach beats Viper's initial contact damage in59/64 (five equal misses/capped hits); Skirmisher beats Hawkeye atContact in59/64, while Hawkeye beats Skirmisher atCover in59/64. No RNG result or damage calculation is replaced. These isolates prove the passive tradeoffs; Viper's later poison, weapon cost/condition and whole-deck survival are separate considerations. Together the broad and complementary tables identify tested contexts for all16families; they do not prove each family has equal overall value.
