...
def candidates(ruleset='legacy'):
    if ruleset=='collection-v1':
        decks={'generalist':['breach','hawkeye','ember'],
               'control':['ramguard','recoil','repulsor'],
               'arcane-bow':['breach','runestring','ember']}
        rows=[dict(id=f'{name}-{path}-{growth}',collection_deck=deck,planner_path=path,
            planner_growth=growth,planner_margin=1,readiness_policy='planner')
            for name,deck in decks.items() for path,growth in product(('bow','staff'),('levels','training'))]
        rows.extend([{**rows[0],'id':'generalist-frontier','planner_margin':0},
                     {**rows[0],'id':'generalist-investor','policy':'investor'},
                     {**rows[4],'id':'control-investor','policy':'investor'},
                     {**rows[0],'id':'generalist-no-gathering','collection_gather':False}])
        return rows
...
def search(*,screen_players=3,screen_days=7,training_seeds=(1701,1702),validation_players=6,
           validation_days=30,validation_seeds=(1801,1802,1803),finalists=3,workers=0,directory=SEARCHES,run_directory=None,grid=None,progress=print,ruleset='legacy'):
    ...
    GameConfig.from_dict({'ruleset':ruleset})
    grid=candidates(ruleset) if grid is None else grid
    ...
    report=dict(schema_version=1,search_id=datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ')+'-'+uuid.uuid4().hex[:8],
        ...
        settings=dict(ruleset=ruleset,screen_players=screen_players,screen_days=screen_days,training_seeds=list(training_seeds),
    ...
    def task(profile,stage,seed,n,days,policy=None,mode='improvements'):
        cfg=GameConfig.from_dict(dict(ruleset=ruleset,players=n,days=days,seed=seed,policies=[policy or profile.get('policy','planner')],probe_mode=mode,workers=1,trace_players=1,
            **{k:v for k,v in profile.items() if k.startswith(('planner_','collection_')) or k=='readiness_policy'}))
    ...
    try:
        ...
        audits=([('ordinary-tactician','tactician','improvements'),('weak-learner','learner','improvements'),('random-experimenter','random','improvements')]
            if ruleset=='collection-v1' else [('historical-mage','mage','session'),('corrected-mage','mage','improvements'),('corrected-learner','learner','improvements')])
        for name,policy,mode in audits:
            profile=dict(id=name)
            if ruleset=='collection-v1':profile['readiness_policy']='planner'
            jobs.extend(task(profile,'validation',seed,validation_players,validation_days,policy,mode) for seed in validation_seeds)
        batch(jobs,workers,receive)
...
def main():
    ...
    p.add_argument('--ruleset',choices=['legacy','collection-v1'],default='legacy')
    p.add_argument('--directory',type=Path,default=SEARCHES)
