...
Set a positive `world_frontier` to keep that frontier fixed for the whole run. Bots may only travel to accessible floors; readiness probes can test later floors on disposable copies. No probe grants its rewards, repaired gear, XP or time to the real simulated player. The default gate is seven wins from eight sampled hunts, not proof of an 80% true win probability. The default `probe_mode=improvements` checks at session boundaries and after equipment, training, level, honing or repair improvements. Successive already-qualified floors receive the same observation time. Probes use legal pack counter-swaps and actual combat hints. Historical `probe_mode=session` retains the old one-floor-per-check timing and older combat decisions for paired auditing; switching modes changes both observation timing and probe competence. A run-end check is also made when no fight is active.
...
Learner uses the lead weapon; Tactician reads the game's attack hints and learns counters; Saver banks and chooses a lower floor; Rusher pushes accessible/deep hunts; Archer and Mage buy/train their chosen path. All start through the actual character-creation flow with normal resources. Purchases are chosen from actual shop availability and actual library prices, then submitted to the engine for validation. Planner additionally uses carried healing, sells redundant equipment, carries counters, learns from losing routes and sleeps between visits. Its weapon focus, investment priority and farming margin are configurable. Field sleep is safe in this personal-state experiment because other players and social ambush services are absent. These are fallible heuristics, not optimal players. They do not read future RNG or copy disposable probe rewards.

Planner decisions are recorded by reason, and refused actions/decision loops are counted. The search refuses to rank a trial containing these failures. Held side blades at range are treated as locked when the game does not offer a legal way to close in with them.
