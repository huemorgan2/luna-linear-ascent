...
| Ordinary hunt entry costs 1 energy; deep entry costs 2. Ordinary attack turns do not each charge this entry cost. Wardens have separate per-strike energy. | Group pricing must state its energy unit. Do not accidentally charge every ordinary swing or give six full old payouts for one old entry price. |
...
Before entry, show the actual group members in order, their images, movement/affinity, visible traits, difficulty, energy cost and reward rules. The player can arrange their deck using what they already own. Browsing or reloading the card cannot reroll the offer. Encounter offers are persisted; deliberate scouting should reveal information, not provide unlimited free lottery rolls.
...
The group should feel like **one scene**. The kill action awards XP, marks that portrait defeated and reveals the next active enemy in the same response. No obligatory loot screen, town visit, new Hunt click or “Continue” click between members. Do not automatically choose the player's next attack.
...
| Successful retreat or rescue extraction | Already earned XP remains | Entire pending haul forfeited | Spent energy, ammunition, consumables and wear remain spent. |
...
| Disconnect, refresh or request retry | No repeated XP | No new roll or payout | Resume the exact encounter; repeat requests return the existing receipt. |
...
Choosing a route to earn easy XP and retreat is allowed. It sacrifices entry energy, time and the haul. We should measure it as a legitimate training strategy, not punish it merely because a player found it. Replaying the **same kill ID** for repeated XP, free entry resets or reopening the same dropped item are bugs.
...
Start QA with **entry energy charged once**, and no new ordinary per-swing energy tax. A simple candidate is `normal entry = ceil(group size / 2)` and `deep entry = 2 × normal entry`: two monsters cost 1/2 energy, three or four cost 2/4, five or six cost 3/6 for normal/deep. This preserves the present two-to-one entry ratio as a starting reference, not a settled price. Authored unusually hard or long groups can have their own displayed price.

Crucially, two enemies for one energy cannot automatically pay twice the current XP, gold and materials. Either that acceleration is an explicit choice, or rewards/threat/entry cost must change together. Freeze the final prices only after simulating a whole account's costs and completion rate. More efficient routes need not be equalized to the average.
...
For another **toy calculation**, assume three identical independent encounters, 80% conditional success each, 10 XP per kill, 100 total gold secured only on completing all three, and two entry energy. Expected XP is `10 × (0.8 + 0.8² + 0.8³) = 19.52`; expected secured gold is `100 × 0.8³ = 51.2`, or 25.6 gold per entry energy before costs. This is not a prediction of Ascent's economy. Real simulations must include the correlation between difficult creatures, valuable loot, survival and resources.
...
| Hunt preview | All group portraits, sequence or waves, floor/biome, profile icons, notable traits, energy cost and a plain “XP per kill; haul after full victory” rule. |
| Combat | Group progress, current enemy and next arrival, three weapon actions, gap, visible enemy intent, statuses/cooldowns, HP and endurance. |
| Kill transition | A compact “enemy 2 of 3 defeated; +XP earned” event and pending haul, with the next enemy ready in the same card. |
...
| Group settings in wiki | Floor, route, roster/order, mode, count, active-enemy limit, arrival rules, energy, reward settings and conditional group rarity chances. |
...
|---|---|
| 1 — Baseline | Measure today's single hunts, action counts/latency, XP clipping, slot spending and energy. Model candidate group completion and three-weapon ownership. |
| 2 — Definitions/migration | Add fixed decks, enemy/group instances, persistent offers, per-kill XP receipts, pending haul, completion settlement and XP reserve. Map School purchases and active legacy fights. |
| 3 — First ten floors | Deliver the full sequential group loop with starter coverage, partial XP, lost haul, resumed groups and first Forge upgrade. This is the first playable milestone. |
...
| 5 — Economy | Simulate three-weapon acquisition/upgrades, group failure, per-enemy drops, secured rewards, XP farming, arrow substitution and deep routes. Retire per-single-monster fairness targets. |
...
| Economy | Track median/P90/P99 time to a functional three-weapon deck, first/next upgrades, grade transitions and recovery. Include uncompleted runs and secured material pairs, not just items rolled. |
...
| Deep hunts | A prepared route shows a real net benefit after failures and costs; wrong decks can perform much worse. Measure each rarity and carrier, not only total gold. |
...
Documentation verification: all 23 local Markdown links across this research and its two updated index/plan documents resolve. The survival, XP, gold and combined-drop examples were recomputed and match the displayed figures. Whitespace checks passed. These checks validate the document, not the proposed game balance.
