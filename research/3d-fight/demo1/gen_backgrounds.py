#!/usr/bin/env python3
"""3d-fight demo1: floor-1 fight backgrounds as looping 1-bit GIFs.

Per fight (floor 1, The Fencerows):
  stills: nano-banana-pro renders the scene as designed 1-bit dither
          art (vision/1bit-images.md headline lesson) — an EMPTY stage
          with a flat fighting floor across the lower quarter.
          -> backgrounds/{id}.jpg (model render, versioned, never overwritten)
  gifs:   local only. The render is downscaled to the 480x270 master
          grid as a greyscale density field, then a 24-frame perfect
          loop is built: wind displacement above the floor line, glow
          pulse in bright regions, cyclic flicker noise where fire
          burns. Fixed Bayer 8x8 per frame (static pixels do not boil).
          -> backgrounds/{id}.gif (1x) + backgrounds/{id}_still.png (2x)
...
FLOOR_FRAC = 0.78          # floor line: combatants' feet at 0.78 * H
BAYER8 = np.array([
...
STAGE = (
    ...
    "later stand — with nothing tall growing or standing on it. It is "
    "real ground, part of the scene: short dithered grass and earth "
    "with a designed gradient pool of light falling on it, NOT a blank "
    "empty band. All hedges, structures and scenery sit BEHIND that "
    "open ground; the ground line is strictly horizontal. "
)

# Floor 1 — The Fencerows. wind: sway amplitude in px; fire: flicker.
SCENES: dict[str, dict] = {
    "grey_wolf": dict(wind=2.2, fire=False, prompt=(
    ...
    "feral_boar": dict(wind=1.8, fire=False, prompt=(
    ...
    "hedge_rat": dict(wind=1.6, fire=False, prompt=(
    ...
    "lane_wolf": dict(wind=2.4, fire=False, prompt=(
        ...
        "grass.")),
    "goblin_straggler": dict(wind=2.0, fire=True, prompt=(
    ...
    "ember_shade": dict(wind=1.8, fire=True, prompt=(
    ...
    "warden": dict(wind=1.0, fire=False, prompt=(
...
async def gen_still(sid: str, key: str) -> str:
    prompt = (STYLE + STAGE + SCENES[sid]["prompt"] +
              " Wide cinematic side-on shot.")
    res = await providers.generate(
        providers.MODELS["nano-banana-pro"], prompt, aspect="16:9",
        api_key=key)
    if "error" in res:
...
def density_master(sid: str) -> np.ndarray:
    """Model render -> 480x270 greyscale density field in [0, 1]."""
...
def noise_fields(n: int = 3) -> list[np.ndarray]:
    ...
    for _ in range(n):
        ...
        f = np.asarray(Image.fromarray((f * 255).astype(np.uint8)).filter(
            ImageFilter.GaussianBlur(3)), dtype=np.float64) / 255.0
...
def wind_shift(grey: np.ndarray, t: int, amp: float) -> np.ndarray:
    ...
    a = amp * height ** 1.4
    ph = 2 * np.pi * t / FRAMES
    off = a * np.sin(ph + ys * 0.028 + xs * 0.010)
    sx = xs + off
